...
//  1. CHAR_ASPECT = 0.50  — correct math for 7px / 8.5px mono
//  2. Rotation bug fixed  — was two assignments, second overwrote
//     first so model barely moved (±10°). Now single oscillation.
//  3. 20fps cap           — ASCII updates invisible above 20fps.
//     Was running at 60fps, wasting 3× CPU/GPU budget.
//  4. Direct DOM mutation — preRef.current.textContent instead
//     of setAscii(). Eliminates 60 React re-renders per second.
//  5. Time-based animation — uses timestamp/1000 so speed is
//     correct at any frame rate, not tied to rAF frequency.
//  6. Portrait lighting   — softer key + fill so original face
//     textures carry the detail (now that we keep them).
// ─────────────────────────────────────────────────────────────
...
// char_width ≈ 7px × 0.6 = 4.2px  |  line-height = 8.5px
// CHAR_ASPECT = 4.2 / 8.5 = 0.494 ≈ 0.50
const CHAR_ASPECT = 0.50;
...
export default function AsciiAvatar({
  ...
  const [loaded, setLoaded] = useState(false);
  // Direct DOM ref — we mutate textContent each frame, bypassing React
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // Reduce resolution on narrow screens
      const isMobile = window.innerWidth < 640;
      const cols     = isMobile ? Math.round(colsProp * 0.60) : colsProp;
      const rows     = isMobile ? Math.round(rowsProp * 0.60) : rowsProp;

      // ── Offscreen GL canvas (1 px = 1 char cell) ───────────
      const glCanvas  = document.createElement("canvas");
      glCanvas.width  = cols;
      glCanvas.height = rows;
      ...
      // Portrait telephoto — 35° FOV, correct char aspect ratio
      const camera = new THREE.PerspectiveCamera(
        35,
        (cols * CHAR_ASPECT) / rows,
        0.1,
      ...
      // ── Portrait lighting ───────────────────────────────────
      // Moderate ambient so original textures stay readable.
      // Lower key intensity than before — textures carry the
      // detail, light should reveal not obliterate.
      scene.add(new THREE.AmbientLight(0xffffff, 0.55));
      ...
      // Frames model with ~20% padding regardless of GLB size.
      const box    = new THREE.Box3().setFromObject(model);
      const sphere = new THREE.Sphere();
      box.getBoundingSphere(sphere);
      ...
      function tick(timestamp: number) {
        ...
        // 20fps cap — ASCII art is imperceptible above 20fps.
        // Reduces CPU+GPU load by ~65% vs uncapped 60fps.
        if (timestamp - lastRender < 50) return;
        lastRender = timestamp;

        if (document.hidden) return;

        // Time-based so animation speed is frame-rate independent
        const t = timestamp / 1000; // seconds since page load

        // FIX: single assignment. Old code had two lines for
        // rotation.y — second overwrote first, leaving ±10° wobble.
        // Now: slow ±30° oscillation over ~12 seconds.
        model.rotation.y = Math.sin(t * 0.52) * 0.52;
        model.rotation.x = Math.sin(t * 0.24) * 0.05;
        neon.intensity    = 0.15 + Math.sin(t * 1.3) * 0.08;
        ...
        // setAscii() was triggering 60 re-renders/sec (huge perf drain).
        if (preRef.current) {
          preRef.current.textContent = pixelsToAscii(imageData, cols, rows);
        }
      }
  ...
  return (
    <div className={`relative inline-block ${className}`} aria-hidden="true">
      ...
      {/* Empty pre — textContent injected directly each frame */}
      <pre
        ref={preRef}
        style={{
